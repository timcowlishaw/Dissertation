...
>    ol_getdepthneartop :: ordertype -> num -> orderlist -> num
...
>ol_getdepthneartop Offer percent ((p,xs):ys) = (sumsizes xs) + (f p ys)
>                                               where
>                                               sumsizes [] = 0
>                                               sumsizes (o:os) = (order_getsize o) + (sumsizes os)
>                                               f p [] = 0
>                                               f p ((p1,xs):ys) = (sumsizes xs) + (f p ys), if (p1<(p*(1+percent)))
>                                                                = 0, otherwise
>ol_getdepthneartop Bid   percent ys          = ol_getdepthneartop Offer percent (reverse ys)
